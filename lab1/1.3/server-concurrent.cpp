...
int main(int argc, char* argv[]) {
  ...
  while (1) {
    ...
    for (size_t i = 0; i < connections.size(); ++i) {
      ...
      max_fd = cd.sock > max_fd ? cd.sock : max_fd;
    ...
    /*
     * man select():
     * "First parameter should be set to the highest-numbered file descriptor in
     * any of the three sets, plus 1...""
     *
     */
    ...
    if (FD_ISSET(listenfd, &readfds)) {
      ...
      if (!set_socket_nonblocking(clientfd)) continue;
#endif
    ...
    bool process_further;
    for (size_t i = 0; i < connections.size(); ++i) {
      ConnectionData *cd = &connections[i];

      if (FD_ISSET(cd->sock, &readfds)) {
        process_further = process_client_recv(*cd);
      }

      else if (FD_ISSET(cd->sock, &writefds)) {
        process_further = process_client_send(*cd);
      }
